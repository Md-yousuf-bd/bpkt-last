            ...
            <div class="col-lg-12 ">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">কোডে বরাদ্দের তালিকা</h5>
                        <div class="card-tools">
                            ...
                            @if(auth()->user()->can('create-code-allotment'))
                                <a href="{{route('code-allotment.create')}}" class="btn btn-sm btn-default pull-right"><span class="fa fa-plus-circle"></span> কোডে বরাদ্দের তথ্য যুক্ত করুন</a>
                            @endif
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="table-responsive" style="font-size: 13px; background: white; padding: 10px;  overflow: auto;">
                            <table id="codeAllotmentTable" class="table table-bordered table-hover table-striped" style="font-size: 14px; width:100%;">
                                <thead>
                                <tr>
                                    <th></th>
                                    <th></th>
                                ...
                                <tr>
                                    ...
                                    <th data-sl="8" >বরাদ্দ তারিখ</th>
                                    <th data-sl="9" >বরাদ্দ স্মারক</th>
                                    <th data-sl="10" >বরাদ্দ স্মারকের তারিখ</th>
...
@section('uncommonInJs')
    <script>
        var codeAllotmentTableColumnNames=[];
        (function() {
            "use strict";

            let codeAllotmentTableColumnTh=document.querySelectorAll('#codeAllotmentTable thead tr:nth-child(2) th');
            for(let i=0; i<codeAllotmentTableColumnTh.length; i++){codeAllotmentTableColumnNames[i]=codeAllotmentTableColumnTh[i].innerText}
            $('#codeAllotmentTable thead tr:nth-child(2) th').each(function () {
                var title = $(this).text();
                let except=['ক্রঃনং','Action','বরাদ্দ তারিখ','অনুমোদনের অবস্থা','অনুমোদনের তারিখ','সর্বশেষ সংস্করণ','তথ্য যুক্ত হয়েছে'];
        ...
        var codeAllotmentTable= $('#codeAllotmentTable').DataTable({
            ...
            ajax: {
                url:'{!! route('code-allotment.get_index') !!}',
                dataType: "json",
            ...
            columns:[
                ...
                {data:'allotment_memo',name:'allotment_memo',class:'text-left'},
                {data:'DT_RowData.data-allotment_memo_date',name:'allotment_memo_date',class:'text-center'},
            ...
            buttons: [
                {
                    extend: 'print',
                    title:'{{ config('app.name', 'Laravel') }}: কোডে বরাদ্দের তালিকা',
                    footer: true,
                    exportOptions: {
                        stripHtml : false,
                        columns: ':visible'
                ...
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        showUserTablesCombinationModal(codeAllotmentTableColumnNames,'বরাদ্দের তালিকা','codeAllotmentTable');
                    }
                },
                {
                    text: 'Reset',
        ...
        codeAllotmentTable.columns().every(function (k) {

            var table = this;
            $('input', this.header()).on('keyup change', function () {
                let columns= codeAllotmentTable.context[0].aoColumns;
                let sl=[];
                ...
                if (table.search() !== this.value || this.value==='') {
                    codeAllotmentTable.columns(a).search(this.value).draw();
                }
            });
        ...
        function set_custom_length()
        {
            ...
            node.type='number';
            node.id='codeAllotmentTable_length_custom';
            ...
            let el = document.getElementById("codeAllotmentTable_length");
            el.querySelector("label").appendChild(button);
            el.querySelector("label").appendChild(node);
        }
        function changeLength(){
            let customLength=document.getElementById("codeAllotmentTable_length_custom").value;
            codeAllotmentTable.page.len(customLength).draw();
        }
        ...
        $(window).on('load',function () {
            $('#codeAllotmentTable').dataTable().fnSort([ [0,'desc']] );
            setTimeout(function () {
                get_set_combination(codeAllotmentTable,'codeAllotmentTable',codeAllotmentTableColumnNames);
            },500);
        });

    </script>
    @include('admin.layouts.commons.modals.user_tables_combination')
    @include('admin.code_allotments.modals.index_filter')
    <script>
        $('#filter_submit').on('click',function () {
            codeAllotmentTable.ajax.reload();
        });
        ...
        function setCustomStatus(){
            setTimeout(function (){
                let status=document.getElementById('codeAllotmentTable_info').innerHTML;
        ...
        function approved(id){
            if(confirm('আপনি কি সত্যিই বরাদ্দটি অনুমোদন করতে চান?')) {
                var csrfToken = $('meta[name="csrf-token"]').attr('content');
                let route = '{{route('code-allotment.approved')}}';
                $.ajax({
                    url: route,
                    data: {
                        id: id,
                    ...
                    success: function (response) {
                        if (response) {
                            ...
                            alertify.notify('বরাদ্দটি অনুমোদিত হয়েছে।', 'success', 5, function () {
                            });
                        } else {
                            alertify.notify('বরাদ্দটি অনুমোদিত হয়নি।', 'error', 5, function () {
                            });
                        }
                    }
                });
        ...
        function unapproved(id){
            if(confirm('আপনি কি সত্যিই বরাদ্দটি অননুমোদন করতে চান?')) {
                var csrfToken = $('meta[name="csrf-token"]').attr('content');
                let route = '{{route('code-allotment.unapproved')}}';
                $.ajax({
                    url: route,
                    data: {
                        id: id,
