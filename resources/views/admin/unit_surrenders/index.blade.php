            ...
            <div class="col-lg-12 ">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">ইউনিটে সমর্পনের তালিকা</h5>
                        <div class="card-tools">
                            ...
                            @if(auth()->user()->can('create-unit-surrender'))
                                <a href="{{route('unit-surrender.create')}}" class="btn btn-sm btn-default pull-right"><span class="fa fa-plus-circle"></span> ইউনিটে সমর্পনের তথ্য যুক্ত করুন</a>
                            @endif
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="table-responsive" style="font-size: 13px; background: white; padding: 10px;  overflow: auto;">
                            <table id="unitSurrenderTable" class="table table-bordered table-hover table-striped" style="font-size: 14px; width:100%;">
                                <thead>
                                <tr>
                                    <th></th>
                                    <th></th>
                                ...
                                <tr>
                                    ...
                                    <th data-sl="9" >সমর্পন তারিখ</th>
                                    <th data-sl="10" >সমর্পনের স্মারক</th>
                                    <th data-sl="11" >সমর্পনের স্মারকের তারিখ</th>
...
@section('uncommonInJs')
    <script>
        var unitSurrenderTableColumnNames=[];
        (function() {
            "use strict";

            let unitSurrenderTableColumnTh=document.querySelectorAll('#unitSurrenderTable thead tr:nth-child(2) th');
            for(let i=0; i<unitSurrenderTableColumnTh.length; i++){unitSurrenderTableColumnNames[i]=unitSurrenderTableColumnTh[i].innerText}
            $('#unitSurrenderTable thead tr:nth-child(2) th').each(function () {
                var title = $(this).text();
        ...
        var unitSurrenderTable= $('#unitSurrenderTable').DataTable({
            ...
            ajax: {
                url:'{!! route('unit-surrender.get_index') !!}',
                dataType: "json",
            ...
            columns:[
                ...
                {data:'surrender_memo',name:'surrender_memo',class:'text-left'},
                {data:'DT_RowData.data-surrender_memo_date',name:'surrender_memo_date',class:'text-center'},
            ...
            buttons: [
                {
                    extend: 'print',
                    title:'{{ config('app.name', 'Laravel') }}: ইউনিটে সমর্পনের তালিকা',
                    footer: true,
                    exportOptions: {
                        stripHtml : false,
                        columns: ':visible'
                ...
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        showUserTablesCombinationModal(unitSurrenderTableColumnNames,'সমর্পনের তালিকা','unitSurrenderTable');
                    }
                },
                {
                    text: 'Reset',
        ...
        unitSurrenderTable.columns().every(function (k) {

            var table = this;
            $('input', this.header()).on('keyup change', function () {
                let columns= unitSurrenderTable.context[0].aoColumns;
                let sl=[];
                ...
                if (table.search() !== this.value || this.value==='') {
                    unitSurrenderTable.columns(a).search(this.value).draw();
                }
            });
        ...
        function set_custom_length()
        {
            ...
            node.type='number';
            node.id='unitSurrenderTable_length_custom';
            ...
            let el = document.getElementById("unitSurrenderTable_length");
            el.querySelector("label").appendChild(button);
            el.querySelector("label").appendChild(node);
        }
        function changeLength(){
            let customLength=document.getElementById("unitSurrenderTable_length_custom").value;
            unitSurrenderTable.page.len(customLength).draw();
        }
        ...
        $(window).on('load',function () {
            $('#unitSurrenderTable').dataTable().fnSort([ [0,'desc']] );
            setTimeout(function () {
                get_set_combination(unitSurrenderTable,'unitSurrenderTable',unitSurrenderTableColumnNames);
            },500);
        });

    </script>
    @include('admin.layouts.commons.modals.user_tables_combination')
    @include('admin.unit_surrenders.modals.index_filter')
    <script>
        $('#filter_submit').on('click',function () {
            unitSurrenderTable.ajax.reload();
        });
        ...
        function setCustomStatus(){
            setTimeout(function (){
                let status=document.getElementById('unitSurrenderTable_info').innerHTML;
