            ...
            <div class="col-lg-12 ">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">কোডে সমর্পণের চিঠির তালিকা</h5>
                        <div class="card-tools">
                            ...
                            @if(auth()->user()->can('create-surrender-letter'))
                                <a href="{{route('code-surrender-letter.create')}}" class="btn btn-sm btn-default pull-right"><span class="fa fa-plus-circle"></span> কোডে সমর্পণের চিঠি যুক্ত করুন</a>
                            @endif
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="table-responsive" style="font-size: 13px; background: white; padding: 10px;  overflow: auto;">
                            <table id="codeSurrenderLetterTable" class="table table-bordered table-hover table-striped" style="font-size: 14px; width:100%;">
                                <thead>
                                <tr>
                                    <th></th>
                                    <th></th>
                                ...
                                <tr>
                                    ...
                                    <th data-sl="2" >নথি নং</th>
                                    <th data-sl="3" >স্মারক</th>
                                    <th data-sl="4" >স্মারক এর তারিখ</th>
                                    <th data-sl="5" >স্বাক্ষর</th>
                                    <th data-sl="6" >সাক্ষরের তারিখ</th>
                                    <th data-sl="7" >মোট সমর্পণ সংখ্যা</th>
                                    <th data-sl="8" >মোট সমর্পণকৃত অর্থের পরিমাণ</th>
                                    <th data-sl="9"  style="min-width: 250px;">চিঠির বিষয়</th>
                                    <th data-sl="10" >মোট প্রেরিত মেইল</th>
                                    <th data-sl="11" >মোট প্রেরিত এসএমএস</th>
...
@section('uncommonInJs')
    <script>
        var codeSurrenderLetterTableColumnNames=[];
        (function() {
            "use strict";

            let codeSurrenderLetterTableColumnTh=document.querySelectorAll('#codeSurrenderLetterTable thead tr:nth-child(2) th');
            for(let i=0; i<codeSurrenderLetterTableColumnTh.length; i++){codeSurrenderLetterTableColumnNames[i]=codeSurrenderLetterTableColumnTh[i].innerText}
            $('#codeSurrenderLetterTable thead tr:nth-child(2) th').each(function () {
                var title = $(this).text();
                let except=['ক্রঃনং','Action','স্মারক এর তারিখ','স্বাক্ষর','সাক্ষরের তারিখ','মোট সমর্পণ সংখ্যা','মোট সমর্পণকৃত অর্থের পরিমাণ','মোট প্রেরিত মেইল','মোট প্রেরিত এসএমএস','সর্বশেষ সংস্করণ','তথ্য যুক্ত হয়েছে'];
        ...
        var codeSurrenderLetterTable= $('#codeSurrenderLetterTable').DataTable({
            ...
            ajax: {
                url:'{!! route('code-surrender-letter.get_index') !!}',
                dataType: "json",
            ...
            columns:[
                ...
                {data:'DT_RowData.data-total_surrenders',name:'total_surrenders',class:'text-center'},
            ...
            buttons: [
                {
                    extend: 'print',
                    title:'{{ config('app.name', 'Laravel') }}: কোডে সমর্পণের চিঠির তালিকা',
                    footer: true,
                    exportOptions: {
                        stripHtml : false,
                        columns: ':visible'
                ...
                },
                {
                    ...
                    action: function ( e, dt, node, config ) {
                        showUserTablesCombinationModal(codeSurrenderLetterTableColumnNames,'সমর্পণের তালিকা','codeSurrenderLetterTable');
                    }
                },
                {
                    text: 'Reset',
        ...
        codeSurrenderLetterTable.columns().every(function (k) {

            var table = this;
            $('input', this.header()).on('keyup change', function () {
                let columns= codeSurrenderLetterTable.context[0].aoColumns;
                let sl=[];
                ...
                if (table.search() !== this.value || this.value==='') {
                    codeSurrenderLetterTable.columns(a).search(this.value).draw();
                }
            });
        ...
        function set_custom_length()
        {
            ...
            node.type='number';
            node.id='codeSurrenderLetterTable_length_custom';
            ...
            let el = document.getElementById("codeSurrenderLetterTable_length");
            el.querySelector("label").appendChild(button);
            el.querySelector("label").appendChild(node);
        }
        function changeLength(){
            let customLength=document.getElementById("codeSurrenderLetterTable_length_custom").value;
            codeSurrenderLetterTable.page.len(customLength).draw();
        }
        ...
        $(window).on('load',function () {
            $('#codeSurrenderLetterTable').dataTable().fnSort([ [0,'desc']] );
            setTimeout(function () {
                get_set_combination(codeSurrenderLetterTable,'codeSurrenderLetterTable',codeSurrenderLetterTableColumnNames);
            },500);
        });

    </script>
    @include('admin.layouts.commons.modals.user_tables_combination')
    @include('admin.code_surrenders.letters.modals.index_filter')
    <script>
        $('#filter_submit').on('click',function () {
            codeSurrenderLetterTable.ajax.reload();
        });
        ...
        function setCustomStatus(){
            setTimeout(function (){
                let status=document.getElementById('codeSurrenderLetterTable_info').innerHTML;
